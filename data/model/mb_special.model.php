...
class mb_specialModel extends Model{
    ...
    public function getMbSpecialItemUsableListByID($special_id) {
        ...
        if(!empty($item_list)) {
            ...
            foreach ($item_list as $value) {
                //处理图片
                $item_data = $this->_formatMbSpecialData($value['item_data'], $value['item_type'],$value['item_content'],$value['item_count']);
     ...
     * 商城页专题
     */
    public function getMbSpecialShopMall() {
    	return $this->getMbSpecialItemUsableListByID(self::GOODS_SPECIAL_ID);
    }
    ...
    private function _formatMbSpecialData($item_data, $item_type,$item_content,$item_count) {
        switch ($item_type) {
            case 'home1':
                ...
                $item_data['goods_count'] = $item_count;
                break;
            case 'home2':
            case 'home4':
     ...
     * 处理专题数据，拼接图片URL
     */
    private function _formatMbSpecialContent($item_content, $item_type) {
        switch ($item_type) {
            case 'home1':
                $item_data['content'] = getMbSpecialImageUrl($item_data['image']);
                break;
            case 'home2':
            case 'home4':
                $item_data['square_image'] = getMbSpecialImageUrl($item_data['square_image']);
    ...
    public function getMbSpecialModuleList() {
        ...
        $module_list['content'] = array('name' => 'content' , 'desc' => '专题内容编辑板块');
