...
class themeControl extends mobileHomeControl{
	...
	public function save_themeOp(){
		...
		if($themeid){
			...
			$model->table('circle_member')->where(array('member_id'=>$_POST['member_id'], 'circle_id'=>1))->update($update);
			...
			$return['status'] = '0';
			$return['message'] = '成功';
			$return['attachment'] = $circle_affix;
	 ...
	 * 转发主题
	 */
	public function forward_themeOp(){
		$model = Model();
		$t_id = $_POST['t_id'];
		if($t_id <= 0){
			$return['status']=1;
			$return['message']='t_id不能为空';
			echo $return;exit;
		}
		//附件
		$affix_list = array();
		...
		if(!empty($theme_info)){
			$affix_list = Model()->table('circle_affix')->where(array('affix_type'=>1, 'circle_id'=>1, 'theme_id'=>$t_id))->select();
			$result['theme_info'] = $theme_info;
			$result['affix_list'] = $affix_list;
		}else{
			$return['status']=1;
			$return['message']='t_id不存在';
			echo $return;exit;
		}
		//存储转发主题
		$insert_t = array();
		$insert_t['theme_content']	= circleCenterCensor($_POST['name']);
		$insert_t['theme_name']= $theme_info['theme_content'];
		$insert_t['member_id']	= $_POST['member_id'];
		$insert_t['member_name']	= $_POST['member_name'];
		$insert_t['theme_addtime']= time();
		$insert_t['lastspeak_time']= time();
		$insert_t['is_forward'] = 1;
		$insert_t['forward_member_id'] = $theme_info['member_id'];
		$insert_t['forward_member_name'] = $theme_info['member_name'];
		$themeid = $model->table('circle_theme')->insert($insert_t);
		//echo json_encode($themeid);exit;
		if($themeid){
			foreach ($affix_list as $key => $affix){
				...
				if($id){
					$data['msg']		= 'success';
					$data['file_url']= themeImageUrl($affix['affix_filename']);
				}
...
// 	}
	...
	public function image_uploadOp(){
		//echo json_encode($_FILES["uploadFiles"]);exit;
		import("manyUploadfile");
		foreach ($_FILES["uploadFiles"]["error"] as $key => $error) {
		    $data['msg']				= 'error';
			$data['origin_file_name']	= $_FILES["uploadFiles"]["name"][$key];
			// 验证已上传附件数量  最大10个
			$where = array();
			$where['member_id']	= $_SESSION['member_id'];
			if($_GET['type'] == 'reply'){
			...
			$count = Model()->table('circle_affix')->where($where)->count();
			if($count < 10){
				$partpath = themePartPath(10);
				$upload = new ManyUploadFile();
				$upload->set('default_dir', ATTACH_CIRCLE.'/theme/'.$partpath);
				//$upload->set('thumb_width',	1024);
				//$upload->set('thumb_height', 160);
				//$upload->set('thumb_ext', '_160x160');
				$file['tmp_name']= $_FILES["uploadFiles"]["tmp_name"][$key];
				$file['name']= $_FILES["uploadFiles"]["name"][$key];
				$file['type']= $_FILES["uploadFiles"]["type"][$key];
				$file['size']= $_FILES["uploadFiles"]["size"][$key];
				$result = $upload->upfile($file);		// 暂时的名字
				//echo json_encode($result);exit;
				if ($result){
					$insert = array();
					$insert['affix_filename']	= $partpath.'/'.$upload->file_name;
					//$insert['affix_filethumb']	= $partpath.'/'.$upload->thumb_image;
					$insert['affix_filesize']	= intval($_FILES["uploadFiles"]["size"][$key]);
					$insert['affix_addtime']	= time();
					$insert['affix_type']		= ($_GET['type'] == 'reply')?2:1;
					$insert['member_id']		= 9;
					$insert['theme_id']			= 0;
	...
	public function save_replyOp(){
		...
		if($reply_id){
			...
			$update['lastspeak_id']			= $_POST['member_id'];
			$update['lastspeak_name']		= $_POST['member_name'];
			...
			if($this->theme_info['member_id'] != $_POST['member_id']){
