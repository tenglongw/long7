...
class member_snsfriendControl extends mobileMemberControl {
	...
	public function addfollowOp() {
		$self_member_id = intval($_POST['member_id']);
		$follow_member_id = intval($_POST['follow_member_id']);
		if($self_member_id<=0){
			echo json_encode(Language::get('wrong_argument'));exit;
		}
		//验证会员信息
		$member_model = Model('member');
		$condition_arr = array();
		$condition_arr['member_state'] = "1";
		$condition_arr['member_id'] = array('in', array($self_member_id,$follow_member_id));
		$member_list = $member_model->getMemberList($condition_arr);
		unset($condition_arr);
		if(empty($member_list)){
			echo json_encode(Language::get('snsfriend_member_error'));exit;
		}
		$self_info = array();
		$member_info = array();
		foreach($member_list as $k=>$v){
			if($v['member_id'] == $self_member_id){
				$self_info = $v;
			}else{
				$member_info = $v;
			}
		...
		$friend_count = $friend_model->countFriend(array('friend_frommid'=>"{$self_member_id}",'friend_tomid'=>"$follow_member_id"));
		if($friend_count >0 ){
			echo json_encode(Language::get('snsfriend_havefollowed'));exit;
		}
		//查询对方是否已经关注我，从而判断关注状态
		$friend_info = $friend_model->getFriendRow(array('friend_frommid'=>"{$follow_member_id}",'friend_tomid'=>"{$self_member_id}"));
		$insert_arr = array();
		...
		if($result){
			...
			$return = json_encode(array('status'=>'0','message'=>'关注成功'));
		}else{
			$return = json_encode(array('status'=>'1','message'=>'关注失败'));
		}
		echo $return;exit;
	}
	/**
