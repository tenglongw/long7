              ...
              <td colspan="15"><h6 class="clear" id="spec_h6_0">默认</h6>
                            <ul class="nofloat">
                            	<?php foreach ($output['spec_list'] as $key=>$val){
									//if($key == $output['type_info']['class_id']){?>
            ...
            <td><h6 class="clear" id="brand_h6_0">默认</h6>
              ...
              <?php 
              	if(is_array($output['brand_list']) && !empty($output['brand_list'])){
              				foreach ($output['brand_list'][$output['type_info']['class_id']]['brand'] as $value){
                            		?>
    ...
    <table class="table tb-type2">
      ...
      <tbody id="tr_model">
        ...
        if(is_array($output['attr_list']) && !empty($output['attr_list'])){
        	foreach ($output['attr_list'] as $value){?>
        	<tr class="hover edit">
          <input type="hidden" value="" name="at_value[<?php echo $value['attr_id']?>][form_submit]" nc_type="submit_value" />
          <input type="hidden" value="<?php echo $value['attr_id']?>" name="at_value[<?php echo $value['attr_id']?>][a_id]" nc_type='ajax_attr_id' />
          <td class="w48"><input type="checkbox" name="a_del[<?php echo $value['attr_id']?>]" value="<?php echo $value['attr_id']?>" /></td>
          <td class="w48 sort"><input type="text" class="change_default_submit_value" name="at_value[<?php echo $value['attr_id']?>][sort]" value="<?php echo $value['attr_sort']?>" /></td>
          <td class="w25pre name"><input type="text" class="change_default_submit_value" name="at_value[<?php echo $value['attr_id']?>][name]" value="<?php echo $value['attr_name']?>" /></td>
          <td class="w50pre name"><?php echo $value['attr_value']?></td>
          <td class="align-center power-onoff"><input type="checkbox" name="at_value[<?php echo $value['attr_id']?>][show]" value="<?php echo $value['attr_show']?>" class="change_default_submit_value" checked="checked" nc_type="checked_show" />
            <input type="hidden"  /></td>
          <td class="w60 align-center"><a href="index.php?act=type&op=attr_edit&attr_id=<?php echo $value['attr_id']?>">编辑</a></td>
        </tr>
		<?php 
        	}}?>
                      </tbody>
