...
class mb_specialControl extends SystemControl{
    ...
    public function index_editOp() {
        ...
        //echo json_encode($model_mb_special::INDEX_SPECIAL_ID);exit;
    ...
    public function goods_editOp() {
    	...
    	$special_item_list = $model_mb_special->getMbSpecialItemListByID($model_mb_special::GOODS_SPECIAL_ID);
    	Tpl::output('list', $special_item_list);
    	Tpl::output('page', $model_mb_special->showpage(2));
    	Tpl::output('module_list', $model_mb_special->getMbSpecialModuleList());
    	Tpl::output('special_id', $model_mb_special::GOODS_SPECIAL_ID);
    	//echo json_encode($model_mb_special::INDEX_SPECIAL_ID);exit;
    	$this->show_menu('index_edit');
    	Tpl::showpage('mb_special_item.list');
    }
    ...
    public function special_item_saveOp() {
        $model_mb_special = Model('mb_special');
        //echo json_encode(array('item_data' => $_POST['item_data'],'item_content' => $_POST['item_content'],'item_count' => count(_POST['item_data']['item'])));exit;
        $result = $model_mb_special->editMbSpecialItemByID(array('item_data' => $_POST['item_data'],'item_content' => $_POST['item_content'],'item_count' => count($_POST['item_data']['item'])), $_POST['item_id'], $_POST['special_id']); 
