...
class store_goods_addControl extends BaseSellerControl {
	...
	private function checkStore() {
		...
		if ($goodsLimit > 0) {
			// 是否到达商品数上限
			$goods_num = Model ( 'goods' )->getGoodsCommonCount ( array (
	...
	public function add_step_oneOp() {
		...
		// echo json_encode($goods_class);exit;
	...
	public function add_step_twoOp() {
		...
		// echo json_encode(checkPlatformStoreBindingAllGoodsClass()); exit;
		// 如果不是自营店铺或者自营店铺未绑定全部商品类目，读取绑定分类
		if (! checkPlatformStoreBindingAllGoodsClass ()) {
			// 商品分类 by w y gk.cn支持批量显示分类
		...
		// 关联版式
		$plate_list = Model ( 'store_plate' )->getStorePlateList ( array (
				'store_id' => $_SESSION ['store_id'] 
	...
	public function save_goodsOp() {
		// echo json_encode($_POST['spec']);exit;
		if (chksubmit ()) {
			// 验证表单
			...
			if ($common_id) {
				...
				showDialog ( L ( 'nc_common_op_succ' ), urlShop ( 'store_goods_add', 'add_step_three', array (
						'commonid' => $common_id 
				) ) );
			} else {
	...
	public function add_step_threeOp() {
		...
		$model_goods = Model ( 'goods' );
		$img_array = $model_goods->getGoodsList ( array (
				'goods_commonid' => $common_id 
		), 'color_id,goods_image', 'color_id' );
		// 整理，更具id查询颜色名称
		// echo json_encode($img_array);exit;
		if (! empty ( $img_array )) {
			$colorid_array = array ();
			$image_array = array ();
		...
		// echo json_encode($value_array);exit;
	...
	public function save_imageOp() {
		if (chksubmit ()) {
			...
			if ($rs) {
				redirect ( urlShop ( 'store_goods_add', 'add_step_four', array (
						'commonid' => $common_id 
				) ) );
			} else {
	...
	public function image_uploadOp() {
		...
		if ($album_limit > 0) {
			$album_count = $model_album->getCount ( array (
					'store_id' => $_SESSION ['store_id'] 
			) );
		...
		$class_info = $model_album->getOne ( array (
				'store_id' => $_SESSION ['store_id'],
				'is_default' => 1 
		), 'album_class' );
		// 上传图片
	...
	public function ajax_stapledelOp() {
		...
		$result = $model_staple->delStaple ( array (
				'staple_id' => $staple_id,
	...
	public function ajax_show_commOp() {
		 ...
		 */
		$model_staple = Model ( 'goods_class_staple' );
		$staple_info = $model_staple->getStapleInfo ( array (
				'staple_id' => intval ( $staple_id ),
				'gc_id_1,gc_id_2,gc_id_3' 
		) );
	...
	public function ajax_get_brandOp() {
		...
		if ($type == 'letter') {
			...
			$where ['brand_name|brand_initial'] = array (
					'like',
					'%' . $keyword . '%' 
			);
		}
